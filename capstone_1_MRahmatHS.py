    ...
    while True:
        ...
        if pilihan == '1':
            if inventaris:
                ...
                print("Pemberitahuan: Inventaris tidak tersedia") #notif jika data tdk ada data tersedia
        elif pilihan == '2':
            nomor_batch = input("Masukkan Nomor Batch item yang ingin dilihat: ").strip().lower()
            tabel = PrettyTable()
            tabel.field_names = ["No", "Nama", "Nomor Batch", "Zat Aktif", "Pabrik", "Stok", "Harga Satuan","Expired Date (YYYY/MM/DD)"]
            for idx in range(len(inventaris)):
                item = inventaris[idx]
                    ...
                    if item['nomor_batch'].lower() == batch_item:
                        ...
                        while True:
                            ...
                            if item['stok'] >= jumlah:
                                ...
                                print(f"Berhasil menambahkan {jumlah} dari {batch_item} ke daftar penjualan. Stok {item['nama']} saat ini: {item['stok']}")
                                ...
                                print(f"Stok tidak mencukupi untuk menjual {jumlah} dari {batch_item}. Stok {item['nama']} saat ini: {item['stok']}")
